...
relkitFull (C.Relkit he1 f1) (C.Relkit he2 f2) _ = 
    do C.Relkit h3 f3 <- relkitMaybe (C.Relkit he2 f2) he1
       ...
       C.Relkit he5 kit5 <- Rop.relkitJoin (C.relkit h4 $ C.RelkitConst bo4) h3
...
consFixJoin use =
  do rmap <- Rop.getRelmap use
     Right $ relmapFix use (Rop.relmapJoin use rmap)
